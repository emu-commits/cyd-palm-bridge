...
void power_busy(int on)                { (void)on; }
int  power_battery_rest_mv(void)       { return 3850; }
int  power_battery_load(void)          { return 0; }
...
void power_stats(PowerStats *st){
    if(!st) return;
    st->mv = 3812; st->pct = 72;      /* mv sags under load; pct comes off rest_mv */
    st->rest_mv = 3850; st->load = 1;
