...
int main(void){
    ...
    CK(geoip_parse("success,42.3601,-71.0589,America/New_York",
                   lat, sizeof lat, lon, sizeof lon, tz, sizeof tz, city, sizeof city) == 1, "a good reply parses");
    ...
    CK(geoip_parse("success,51.5074,-0.1278,Europe/London",
                   lat, sizeof lat, lon, sizeof lon, tz, sizeof tz, city, sizeof city) == 1, "London parses");
    ...
    CK(geoip_parse("success,35.6895,139.6917,Asia/Tokyo\n",
                   lat, sizeof lat, lon, sizeof lon, tz, sizeof tz, city, sizeof city) == 1, "trailing newline ok");
    ...
    CK(geoip_parse("fail,private range", lat, sizeof lat, lon, sizeof lon, tz, sizeof tz, city, sizeof city) == 0,
    ...
    CK(geoip_parse("success,42.3601", lat, sizeof lat, lon, sizeof lon, tz, sizeof tz, city, sizeof city) == 0,
    ...
    CK(geoip_parse("success,N/A,N/A,", lat, sizeof lat, lon, sizeof lon, tz, sizeof tz, city, sizeof city) == 0,
       "non-numeric coordinates rejected");
    CK(geoip_parse("<html><head><title>Redirect", lat, sizeof lat, lon, sizeof lon, tz, sizeof tz, city, sizeof city) == 0,
       "an HTML body is rejected");
    CK(geoip_parse("", lat, sizeof lat, lon, sizeof lon, tz, sizeof tz, city, sizeof city) == 0, "empty body rejected");

    /* a usable coordinate with an unusable zone is still a win */
    CK(geoip_parse("success,-33.8688,151.2093,", lat, sizeof lat, lon, sizeof lon, tz, sizeof tz, city, sizeof city) == 1,
    ...
    CK(geoip_parse("success,-33.8688,151.2093,not a zone",
                   lat, sizeof lat, lon, sizeof lon, tz, sizeof tz, city, sizeof city) == 1, "a spaced zone is ignored");
    CK(tz[0] == 0, "...rather than written into config.ini");

    /* the city name, and the reason it is the LAST field: a place name may
     * contain a comma, and a field read to end-of-line cannot be cut by one */
    CK(geoip_parse("success,42.3601,-71.0589,America/New_York,Boston",
                   lat, sizeof lat, lon, sizeof lon, tz, sizeof tz, city, sizeof city) == 1,
       "a reply with a city parses");
    CK(!strcmp(city, "Boston"), "the city comes back");
    CK(geoip_parse("success,38.9072,-77.0369,America/New_York,Washington, D.C.\n",
                   lat, sizeof lat, lon, sizeof lon, tz, sizeof tz, city, sizeof city) == 1,
       "a city containing a comma parses");
    CK(!strcmp(city, "Washington, D.C."), "...and is not cut at the comma");
    CK(geoip_parse("success,-33.8688,151.2093,Australia/Sydney",
                   lat, sizeof lat, lon, sizeof lon, tz, sizeof tz, city, sizeof city) == 1,
    ...
    CK(strstr(u, "fields=status,lat,lon,timezone,city") != NULL,
