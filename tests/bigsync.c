 ...
 * sizing on the host. The old array-based reconcile capped a collection at
 * MAXR=24 records (the loc/map/srv arrays had to coexist with the TLS handshake).
 * The streaming merge-join has no such cap, so this test syncs 200 records --
 * ~8x the old wall -- each carrying a ~300-byte note, and asserts they all sync,
 * converge, and are idempotent.
 *
...
int main(void){
    ...
    printf("== bigsync: %d records, device sizing (streaming, was capped at 24) ==\n",N);
    clearColl();
    writeDB(N);
