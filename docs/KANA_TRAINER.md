...
**one expected next stroke**; a match advances, a wrong shape/direction/order is
rejected ("follow the model"), and completing every stroke cleanly promotes the
kana (an error-marked run demotes). Stroke order is enforced because matching only
ever targets the current stroke. Write has its own SRS state per kana, persisted
alongside Sound (`/sdcard/kana_train.dat`, magic `KT02`).
...
- **Emulator-verified:** stroke 1 accepted → advances; a wrong stroke rejected +
  held; a faithful trace of all of あ's strokes (incl. the loop) → "Correct! Lv 2"
  and deterministic advance to い. Pool-safe throughout.
